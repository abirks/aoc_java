...
public class AdventOfCode2019Day24 implements NewAocPuzzle {
    private static final char BUG = '#';
    private static final int PART2_MINUTES = 200;

    private boolean[][] initial = new boolean[Grid.DIMENSION][Grid.DIMENSION];

    @Override
    public void load() throws AocLoadException {
        AocInput input = new AocInput(2019, 24);
        char[][] inputArray = input.read2dArray();

        for (int y = 0; y < inputArray.length; y++) {
            for (int x = 0; x < inputArray[y].length; x++) {
                initial[y][x] = inputArray[y][x] == BUG;
            }
        }

        if (initial.length != 5 || initial[0].length != Grid.DIMENSION) {
    ...
    public String part1() throws AocSolveException {
        Grid grid = new Grid(initial, false);
    ...
    public String part2() throws AocSolveException {
        Grid grid = new Grid(initial, true);
